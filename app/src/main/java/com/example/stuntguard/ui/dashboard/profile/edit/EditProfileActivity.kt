...
import com.example.stuntguard.databinding.ActivityEditProfileBinding
        ...
        binding.btnEditprofile.setOnClickListener {
            profileviewModel.getUserData().observe(this) { user ->
                ...
                if (parentName.isNotEmpty() && email.isNotEmpty() && isEmailValid(email) && isNumberValid(phone)) {
                    profileviewModel.updateProfile(user.token, parentId, parentName, email, phone)
                    startActivity(Intent(this, ProfileActivity::class.java))
                    finish()
                } else if (parentName.isNotEmpty() && email.isNotEmpty() && isEmailValid(email)) {
                    profileviewModel.getDetailUser(user.token, user.userId)
                    profileviewModel.detailUser.observe(this) {
                        profileviewModel.updateProfile(user.token, parentId, parentName, email, it.data.phone)
                        startActivity(Intent(this, ProfileActivity::class.java))
                        finish()
                    }
                }
